...
This creates `broadcast_trace.pkl` containing the execution trace in pickle format (compact and fast).
...
Dapyview automatically detects pickle or JSON format.

**Trace Format Options:**

By default, `save_trace()` uses pickle format (`.pkl`):
- **Pickle (default)**: Compact, fast, no dependencies
- **JSON (optional)**: Human-readable, requires `dapy[json]`

```python
# Pickle (recommended)
sim.save_trace("trace.pkl")  # or save_trace("trace.pkl", format="pickle")

# JSON (for inspection/sharing)
sim.save_trace("trace.json", format="json")  # requires: uv pip install dapy[json]
```
...
- **For JSON**: Use a JSON validator on the trace file
- **For pickle**: Verify file is not corrupted
...
Trace files can be in pickle (`.pkl`, default) or JSON (`.json`) format.

Pickle format is binary and compact. JSON format has this structure:
